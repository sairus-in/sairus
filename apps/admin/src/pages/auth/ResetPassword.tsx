...
export const ResetPassword: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      ...
      try {
        const response = await api.post<AdminTokenVerificationResponse>('/v1/admin/auth/reset-password/verify-token', {
          token,
        });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await api.post<{ message: string }>('/v1/admin/auth/reset-password', {
        token,
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '2rem', background: 'linear-gradient(160deg, #DBEAFE 0%, #F8FAFC 45%, #DCFCE7 100%)' }}>
      <div style={{ width: '100%', maxWidth: 480, background: 'rgba(255,255,255,0.96)', border: '1px solid rgba(148,163,184,0.25)', borderRadius: 24, boxShadow: '0 24px 80px rgba(15,23,42,0.12)', padding: '2rem' }}>
        ...
        <h1 style={{ margin: '0.5rem 0 0', fontSize: '2rem', color: '#0F172A' }}>Choose a new password</h1>
        <p style={{ margin: '0.75rem 0 1.5rem', color: '#475569', lineHeight: 1.6 }}>
          {verified ? `Resetting access for ${verified.name}.` : 'We are validating the reset link before allowing the password change.'}
        </p>

        {loading ? (
          <div style={{ color: '#475569' }}>Validating reset link...</div>
        ) : (
